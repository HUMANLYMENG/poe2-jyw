...
`,b="https://www.pathofexile.com/api/trade2",A=null,q=0;async function L(){if(A&&Date.now()-q<36e5)return A;try{let e=await S("statsCache");if(e.statsCache?.entries&&e.statsCache?.timestamp&&Date.now()-e.statsCache.timestamp<36e5){let t=e.statsCache.entries,a=Array.isArray(t)&&t.length>100&&t.every(e=>"string"==typeof e.id&&e.id.length>3);if(a)return A=t,q=e.statsCache.timestamp,A;console.warn("[PoE2] Stats cache corrupted, clearing and re-fetching"),T({statsCache:null}).catch(()=>{})}}catch(e){}let e=await fetch(`${b}/data/stats`);if(!e.ok)throw Error(`Stats API returned ${e.status}`);let t=await e.json(),a=[];for(let e of t.result||[])for(let t of e.entries||[])a.push({id:t.id,text:t.text,type:t.type,option:t.option});return A=a,T({statsCache:{entries:a,timestamp:q=Date.now()}}).catch(()=>{}),a}async function C(e,t){let a=await fetch(`${b}/search/${t}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});if(!a.ok){let e;let t=await a.text();try{let i=JSON.parse(t),n=i.error||"";e=n.includes("Invalid query")?"\u67e5\u8be2\u65e0\u6548 \u2014 \u53ef\u80fd\u662f\u7f13\u5b58\u6570\u636e\u635f\u574f\u3002\u8bf7\u5c1d\u8bd5\uff1a\u6269\u5c55\u7ba1\u7406 \u2192 \u6e05\u9664\u5b58\u50a8 \u2192 \u91cd\u65b0\u52a0\u8f7d\u9875\u9762\u3002":n.includes("too complex")||n.includes("reduce")?"\u67e5\u8be2\u592a\u590d\u6742\uff0c\u5339\u914d\u7ed3\u679c\u592a\u591a\u3002\u8bf7\u52a0\u4e0a\u66f4\u591a\u9650\u5236\u6761\u4ef6\uff08\u6bd4\u5982\u4ef7\u683c\u8303\u56f4\u3001\u5177\u4f53\u7684\u88c5\u5907\u7c7b\u578b\uff09\u6765\u7f29\u5c0f\u7ed3\u679c\u3002":`API \u9519\u8bef (${a.status}): ${n}`}catch{e=`\u641c\u7d22\u5931\u8d25 (${a.status})\uff0c\u8bf7\u91cd\u8bd5\u6216\u5237\u65b0\u9875\u9762\u3002`}throw Error(e)}return a.json()}async function k(e,t){let a=await fetch(`${b}/fetch/${e.join(",")}?query=${t}`);if(!a.ok){let e=await a.text();throw Error(`Fetch API returned ${a.status}: ${e.slice(0,200)}`)}return a.json()}let O=`You are a Path of Exile 2 trade search parser. Convert the user's Chinese natural language search into a JSON search intent.
